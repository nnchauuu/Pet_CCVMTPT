...
const StaffManagement = () => {
  ...
  const fetchStaff = async () => {
    try {
      ...
      if (activeTab === "deleted") {
        url = `${BACKEND_URL}/api/staff/deleted?${params}`;
      } else {
        params.append("sortBy", sortBy);
        params.append("sortDir", sortDir);
        if (search) params.append("search", search);
        url = `${BACKEND_URL}/api/staff/paginated?${params}`;
      }
  ...
  const handleEdit = (staffMember) => {
    ...
    setImagePreview(
      staffMember.profilePictureUrl
        ? `${BACKEND_URL}${staffMember.profilePictureUrl}`
        : null,
    );
    setShowModal(true);
  };
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setImagePreview(`${BACKEND_URL}${result.data}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editingStaff) {
        response = await fetch(`${BACKEND_URL}/api/staff/${editingStaff.id}`, {
          method: "PUT",
          headers: getAuthHeaders(),
          credentials: "include",
        ...
        response = await fetch(`${BACKEND_URL}/api/staff`, {
          method: "POST",
          headers: getAuthHeaders(),
          credentials: "include",
          body: JSON.stringify(formData),
    ...
    showConfirm(`Bạn có chắc muốn ${action} nhân viên này?`, async () => {
      try {
        const response = await fetch(
          `${BACKEND_URL}/api/staff/toggle-active?id=${staffMember.id}`,
          {
            method: "PATCH",
            headers: getAuthHeaders(),
            credentials: "include",
  ...
  const handleDelete = (staffMember) => {
    showConfirm("Bạn có chắc muốn xóa nhân viên này?", async () => {
      try {
        const response = await fetch(
          `${BACKEND_URL}/api/staff/${staffMember.id}`,
          {
            method: "DELETE",
            headers: getAuthHeaders(),
            credentials: "include",
  ...
  const handleRestore = (staffMember) => {
    showConfirm("Bạn có chắc muốn khôi phục nhân viên này?", async () => {
      try {
        const response = await fetch(
          `${BACKEND_URL}/api/staff/restore?id=${staffMember.id}`,
          {
            method: "PATCH",
            headers: getAuthHeaders(),
            credentials: "include",
                    ...
                    staff.map((s) => (
                      <tr key={s.id}>
                        <td className="text-center">
                          {s.profilePictureUrl ? (
                            <img
                              src={`${BACKEND_URL}${s.profilePictureUrl}`}
                              alt={s.fullName}
                ...
                <div className="row">
                  <div className="col-md-4 text-center">
                    {viewingStaff.profilePictureUrl ? (
                      <img
                        src={`${BACKEND_URL}${viewingStaff.profilePictureUrl}`}
